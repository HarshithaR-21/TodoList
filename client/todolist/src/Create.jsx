...
function Create(){
    ...
    useEffect(()=>{
        ...
        if(userEmail){
            axios.get(`http://127.0.0.1:8000/todos/${userEmail}`)
    ...
    const handleAdd = () =>{
        ...
        axios.post('http://127.0.0.1:8000/add', {email: userEmail, task: task}).then(result => {
            console.log(result);
    ...
    const handleDone = (taskDone, index) => {
        const userEmail = localStorage.getItem('userEmail');
        axios.post('http://127.0.0.1:8000/delete', {email: userEmail, taskDone}).then(result => {
            console.log(result);
    ...
    return(
        <div>
            <button className="bg-red-500 text-white font-bold p-3 m-4 text-xl rounded-lg" onClick={handleLogout}>Logout</button>
